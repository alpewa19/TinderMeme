...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (savedUserId) {
        try {
            const response = await fetch(`${API_BASE_URL}/api/users/${savedUserId}`);
            if (response.ok) {
...
async function loadRandomMeme() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/memes/random?userId=${currentUser._id}`);
...
async function handleLike() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/users/${currentUser._id}/action`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function renderFavorites() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/users/${currentUser._id}/favorites`);
...
async function addMeme() {
    ...
    try {
        ...
        if (imagePrompt) {
            ...
            if (!response.ok) {
                throw new Error('Ошибка при генерации мема');
            }
            
            const blob = await response.blob();
            imageUrlToSave = URL.createObjectURL(blob);
            
            // Показываем уведомление об успешной генерации
            showNotification('Мем успешно сгенерирован!', 'success');
            
            // Создаем и показываем мем
            const memeContainer = document.createElement('div');
            memeContainer.className = 'meme-container';
            memeContainer.innerHTML = `
                <img src="${imageUrlToSave}" alt="Сгенерированный мем" class="generated-meme">
                <div class="meme-info">
                    <p class="meme-description">${description || 'Без описания'}</p>
                    <div class="meme-tags">
                        ${tags.map(tag => `<span class="meme-tag">${tag}</span>`).join('')}
                    </div>
                </div>
            `;
            
            // Добавляем мем в основной контейнер
            const mainContainer = document.querySelector('.main-container');
            mainContainer.insertBefore(memeContainer, mainContainer.firstChild);
            ...
            showNotification('Мем успешно загружен!', 'success');
            
            // Создаем и показываем мем
            const memeContainer = document.createElement('div');
            memeContainer.className = 'meme-container';
            memeContainer.innerHTML = `
                <img src="${imageUrl}" alt="Загруженный мем" class="generated-meme">
                <div class="meme-info">
                    <p class="meme-description">${description || 'Без описания'}</p>
                    <div class="meme-tags">
                        ${tags.map(tag => `<span class="meme-tag">${tag}</span>`).join('')}
            ...
            throw new Error('Необходимо указать URL изображения или промпт для генерации');
        }

        // Сохранение мема в базу данных
        const memeData = {
        ...
        if (!saveResponse.ok) {
            throw new Error('Ошибка при сохранении мема');
        }
